...
interface ASCIICapitelSceneProps {
    scale: number;
    rotationY: number;
    cellSize: number;
}
...
const ASCIICapitelScene = ({
    scale,
    rotationY,
    cellSize,
}: ASCIICapitelSceneProps) => {
    ...
    // Create cloned model with custom material and pivot group - focusing on capitel (top)
    const { clonedModel, pivotGroup } = useMemo(() => {
        ...
        const normalizedScale = 5.0 / maxDim;

        // Scale
        clone.scale.setScalar(normalizedScale * scale);

        // Position to show only the top (capitel)
        const topY = box.max.y;
        clone.position.set(
            -center.x * normalizedScale * scale,
            -(topY * 0.92) * normalizedScale * scale,
    ...
    }, [gltfScene, objectMaterial, scale, rotationY]);
...
// Preload helper
export const preloadCapitelModel = () => {
