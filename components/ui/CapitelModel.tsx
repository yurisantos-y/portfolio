...
const asciiFragmentShader = `
    ...
    // Better sampling for anti-aliasing
    float sampleBrightness(sampler2D tex, vec2 uv, vec2 texelSize) {
        float total = 0.0;
        total += texture2D(tex, uv).r;
        total += texture2D(tex, uv + vec2(texelSize.x, 0.0)).r * 0.5;
        total += texture2D(tex, uv - vec2(texelSize.x, 0.0)).r * 0.5;
        total += texture2D(tex, uv + vec2(0.0, texelSize.y)).r * 0.5;
        total += texture2D(tex, uv - vec2(0.0, texelSize.y)).r * 0.5;
        return total / 3.0;
    }
    ...
    void main() {
        ...
        // Calculate average brightness from 5-point sample
...
export const CapitelModel = ({
    ...
    scale = 1,
    rotationY = 0,
    cellSize = 3.0, // Smaller cells for higher density on capitel
}: CapitelModelProps) => {
    ...
    useEffect(() => {
        ...
        loader.load(
            ...
            (gltf) => {
                ...
                // Scale to fit - focusing on the capitel (top)
                const maxDim = Math.max(size.x, size.y, size.z);
                const baseScale = (5.0 / maxDim) * scale;
