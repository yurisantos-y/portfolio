...
const asciiFragmentShader = `
  ...
  uniform float uTime;
  uniform vec2 uMousePos; // Cursor in UV space (0-1)
  uniform float uDispersalAmount; // 0 to 1
  ...
  void main() {
    ...
    // Mouse position is in UV space (0 to 1)
    
    vec2 toMouse = cellCenter - uMousePos;
    // Adjust logic to handle mouse position outside [0,1] gracefully or just use large distance
    
    // Calculate distance scaling for aspect ratio to make interaction circular
    vec2 toMouseScaled = toMouse * vec2(aspectRatio, 1.0);
    float distToMouse = length(toMouseScaled);
    
    // Dispersal parameters
    float dispersalRadius = 0.35; // Interaction radius (increased)
    float dispersalStrength = 0.25; // How far particles fly (increased)
    ...
    // We use the ORIGINAL UV relative to the grid for character lookup
    // BUT mapped to valid char uvs.
    // Important: We want the character to STAY in its grid cell but the content to move?
    // OR we want the character grid itself to disperse?
    // For proper particle effect, the grid moves.
    ...
    // Base alpha from character and brightness - stronger effect
...
interface ASCIIParallaxModelProps {
    path: string;
...
export const ASCIIParallaxModel = ({
    path,
    position = [0, 0, 0],
    rotation = [0, 0, 0],
    scale = 1,
    ...
    const uMousePos = useRef(new THREE.Vector2(-1, -1)); // Start outside
    ...
    // Create cloned model with custom material
    const clonedModel = useMemo(() => {
        ...
        clone.position.sub(center.multiplyScalar(normalizedScale * scale));
        clone.position.set(position[0], position[1], position[2]);
        clone.rotation.set(rotation[0], rotation[1], rotation[2]);

        return clone;
    }, [gltfScene, objectMaterial, scale, position, rotation]);

    // Add model to virtual scene
    useEffect(() => {
        virtualScene.add(clonedModel);
        return () => {
            virtualScene.remove(clonedModel);
        };
    }, [virtualScene, clonedModel]);
    ...
    useFrame((state, delta) => {
        ...
        // Oscillate the model
        if (oscillate && clonedModel) {
            const maxAngle = (oscillationAngle * Math.PI) / 180;
            const baseRotationY = rotation[1] + Math.sin(time * oscillationSpeed) * maxAngle;
            clonedModel.rotation.y = baseRotationY;
        }
    ...
    return (
        <group ref={groupRef}>
            ...
            <mesh
                ...
                onPointerMove={(e) => {
                    // Normalize UV coordinates from intersection point
                    if (e.uv) {
...
// Preload helper
export const preloadASCIIModel = (path: string) => {
